...
public class Maze {
		...
		for (int row = 0; row < numRows; row++) {
			...
			for (int col = 0; col < numCols; col++) {
				if (cells[row][col] != null) {
					if (row > 0 && cells[row - 1][col] != null) { //left
						cells[row][col].addNeighbor(cells[row - 1][col]);
					}
					if (col > 0 && cells[row][col - 1] != null) { //top
						cells[row][col].addNeighbor(cells[row][col - 1]);
					}
					if (row < numRows - 1 && cells[row + 1][col] != null) { //right
						cells[row][col].addNeighbor(cells[row + 1][col]);
					}
					if (col < numCols - 1 && cells[row][col + 1] != null) { //bottom
						cells[row][col].addNeighbor(cells[row][col + 1]);
					}
				}
			}
	...
	public List<MazeNode> dfs(int startRow, int startCol, int endRow, int endCol) {
		...
		Stack<MazeNode> exploreStack = new Stack<MazeNode>();
		exploreStack.push(start);
		boolean found = false;

		// Do the search
		while (!exploreStack.empty()) {
			MazeNode curr = exploreStack.pop();
			if (curr == goal) {
				found = true;
				break;
			}
	...
	public List<MazeNode> bfs(int startRow, int startCol, int endRow, int endCol) {
		...
		HashSet<MazeNode> visited = new HashSet<MazeNode>();
		Queue<MazeNode> exploreQueue = new LinkedList<MazeNode>();
		HashMap<MazeNode, MazeNode> parentMap = new HashMap<MazeNode, MazeNode>();
		exploreQueue.add(start);
		boolean found = false;
		while (!exploreQueue.isEmpty()) {
			MazeNode curr = exploreQueue.remove();
			if (curr == goal) {
				found = true;
				break;
			}
